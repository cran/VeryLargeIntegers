...
#' @description vli is a S3 class that allows to store and operate with arbitrarily large integers. Each object of class vli has 3 attributes: \code{sign}, \code{length} and \code{value} that can be accesed as showed in the examples. The (absolute) value of the number is stored in a vector to avoid truncation.
